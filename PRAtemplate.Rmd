...
(@) Taxonomy:
*e.g. Genus, species/ subspecies, Authority, Family, Order, Kingdom. 
...
```{r echo = F, eval = T}
Q3 <- data.frame(yes = 'Yes',
                 no = 'No')
kable(Q3) %>%
  kable_styling(full_width = F, position = 'left') %>%
  row_spec(row = 0, color = 'white') %>%
  column_spec(c(1, 2), width = '5em')
```

(@) Is the vector needed for pest entry or spread?

```{r echo = F, eval = T}
Q4 <- data.frame(yes = 'Yes',
                 no = 'No')
kable(Q4) %>%
  kable_styling(full_width = F, position = 'left') %>%
  row_spec(row = 0, color = 'white') %>%
...
```{r echo = F, eval = T}
Q7 <- read.csv('habitats.csv', header = T)
