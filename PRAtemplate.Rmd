---
title: "Pest Risk Assessment for"
subtitle: "Pest name"
author: 
  - "Michal Czyz"
institute: "Institute of Magick Research"
keywords:
...
kable(Q8A, col.names = c('Possible pathway',
               'Pathway:')) %>%
  kable_styling(full_width = T, bootstrap_options = c('striped', 'condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
...
cat("Repeat table for each pathway considered.")
```
...
```{r echo = F, eval = T}
Q10 <- data.frame(Rating = c('Rating of the likelihood of establishment in
                            the managed environment',
                            'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for likelihood, second
                 High = c('',''))     #for uncetainity (see example Q9)
...
```{r echo = F, eval = T}
Q11 <- data.frame(Rating = c('Rating of the magnitude of spread in
                            the PRA area',
                            'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
...
```{r echo = F, eval = T}
Q12_1 <- data.frame(Rating = c('Rating of magnitude of impact on biodiversity in
                               the current area of distirbution',
                               'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
 kable(Q12_1, align = c('l', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T, bootstrap_options = c('condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
  column_spec(2:4, width = '8em', bold = T) %>%
  row_spec(1:2, color = 'black', background = '#f0ae82')
...
```{r echo = F, comment = NULL, class.output = 'bg-info', class.output = 'bg-info'}
cat("Examples of ecosystem services to consider under each category include:\n\n*Provisioning services\t\t\t\t\t\t\t\t*Supporting services
...
```{r echo = F, eval = T}
Q12_2A <- data.frame(Rating = c('Rating of magnitude of impact on ecosystem
                                services in the current area of distirbution',
                                'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
 kable(Q12_2A, align = c('l', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T, bootstrap_options = c('condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
  column_spec(2:4, width = '8em', bold = T) %>%
  row_spec(1:2, color = 'black', background = '#f0ae82')
...
```{r echo = F, eval = T}
Q12_3 <- data.frame(Rating = c('Rating of magnitude of socio-economic impact in
                               the current area of distirbution',
                               'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
 kable(Q12_3, align = c('l', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T, bootstrap_options = c('condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
  column_spec(2:4, width = '8em', bold = T) %>%
  row_spec(1:2, color = 'black', background = '#f0ae82')
...
```{r echo = F, eval = T}
Q13_1 <- data.frame(Rating = c('Rating of magnitude of impact on biodiversity in
                               the area of potential establishment',
                               'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
 kable(Q13_1, align = c('l', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T, bootstrap_options = c('condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
  column_spec(2:4, width = '8em', bold = T) %>%
  row_spec(1:2, color = 'black', background = '#f0ae82')
...
```{r echo = F, eval = T}
Q13_2 <- data.frame(Rating = c('Rating of magnitude of impact on ecosystem
                               services in the area of potential establishment',
                               'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
 kable(Q13_2, align = c('l', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T, bootstrap_options = c('condensed', 'bordered')) %>%
  row_spec(row = 0, bold = T, background = '#f5f6fa') %>%
  column_spec(2:4, width = '8em', bold = T) %>%
  row_spec(1:2, color = 'black', background = '#f0ae82')
...
```{r echo = F, eval = T}
Q13_1 <- data.frame(Rating = c('Rating of magnitude of socio-economic impact on
                               biodiversity in the area of potential establishment',
                               'Rating of uncertainty'),
                 Low = c('',''),      #put X in correct level between '' 
                 Moderate = c('',''), #first place is for magnitude, second
                 High = c('',''))     #for uncetainity (see example Q9)
...
```{r echo = F, comment = NULL, class.output = 'bg-info', class.output = 'bg-info'}
cat("In particular, consider the following aspects:\n
...
\tInter-specific competition\t\t\t\t\t\tPer capita effects"
)
```
...
```{r echo = F, comment = NULL, class.output = 'bg-info', class.output = 'bg-info'}
cat('As described in PM 5/3 possible options for phytosanitary measures include 
